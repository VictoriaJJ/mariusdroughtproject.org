...
// 	"Oak in the 2080s"];
var keyArray3 = [
	{"id": "V1", "label": "Beech in the 2030s"},
	{"id": "V2", "label": "Oak in the 2030s"},
	{"id": "V3", "label": "Beech in the 2080s"},
	{"id": "V4", "label": "Oak in the 2080s"}];
...
function setMap(){ //set choropleth map parameters	
	...
	queue() //use queue.js to parallelize asynchronous data loading for cpu efficiency
		.defer(d3.csv, "data/ThamesMortalityvsBaseline.csv") //load attributes data from csv
		.defer(d3.json, "data/ThamesMortalityvsBaseline.topojson") //load geometry from countries topojson
		// .defer(d3.json, "data/FranceRegions.topojson") //load geometry from regions topojson
		.await(callback);

	// function callback(error, csvData, europeData, thamesData){
	function callback(error, csvData, thamesData){
		var recolorMap = colorScale(csvData); //retrieve color scale generator

		//variables for csv to json data transfer
		var jsonAreas = thamesData.objects.ThamesMortalityvsBaseline_epsg4326.geometries;
		...
		for (var i=0; i<csvData.length; i++) {		
			...
			var areaIdField = csvIdField.uid; //id code
			...
			for (var a=0; a<jsonAreas.length; a++){
				...
				if (jsonAreas[a].properties.uid == areaIdField){
		...
		var areas = map.selectAll(".areas")
			.data(topojson.feature(thamesData, thamesData.objects.ThamesMortalityvsBaseline_epsg4326).features) //bind regions data to path element
			.enter() //create elements
			.append("path") //append elements to svg
			.attr("class", "areas") //assign class for additional styling
			.attr("id", function(d) { return d.properties.uid })
			.attr("d", path) //project data as geometry in svg
...
function createDropdown(csvData){
	...
	var dropdown = d3.select("body")
		...
		.html("Percentage change of mortality relative to baseline at 50th percentile:<br>")
		.append("select")
...
function highlight(data){
	...
	d3.select("#"+props.uid) //select the current region in the DOM
	...
	var labelName = props.CAMS_NAME //html string for name to go in child div
	
	//create info label div
	var infolabel = d3.select("body")
		.append("div") //create the label div
		.attr("class", "infolabel")
		.attr("id", props.uid+"label") //for styling label
...
function dehighlight(data){
	var props = data.properties; //json properties
	console.log(data.properties);
	var area = d3.select("#"+props.uid); //select the current area
	...
	d3.select("#"+props.uid+"label").remove(); //remove info label
};
